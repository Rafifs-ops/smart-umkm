<template>
  <div
    class="flex flex-col items-center min-h-screen p-6 text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600"
  >
    <div class="w-full max-w-md mt-6">
      <div class="grid grid-cols-2 gap-4 mb-8">
        <div
          class="p-4 border bg-green-900/40 backdrop-blur-sm rounded-2xl border-cyan-500/30"
        >
          <p class="text-xs text-white">Total Penjualan</p>
          <p class="mt-1 text-xl font-bold text-white">
            Rp {{ totalPenjualan.toLocaleString("id-ID") }}
          </p>
        </div>
        <div
          class="p-4 border bg-green-900/40 backdrop-blur-sm rounded-2xl border-green-500/30"
        >
          <p class="text-xs text-white">Keuntungan</p>
          <p class="mt-1 text-xl font-bold text-white">
      ...
      <button
        @click="mulaiScan"
        class="flex items-center justify-center w-full gap-3 py-5 text-lg font-bold transition-all bg-green-900/50 border rounded-2xl border-cyan-500/30 text-white shadow-[0_0_15px_rgba(6,182,212,0.3)] hover:shadow-[0_0_25px_rgba(6,182,212,0.6)] active:scale-95"
      >
        <svg
          class="w-7 h-7"
          fill="none"
      ...
      <div v-if="barangDitemukan?.length > 0" class="flex flex-col space-y-2">
        <div
          v-for="produk in barangDitemukan"
          class="p-6 mt-8 border bg-green-900/40 backdrop-blur-sm rounded-2xl border-green-500/30 shadow-lg"
        >
          <div class="flex justify-between items-start">
            <div>
              <p class="mb-1 text-sm text-white">Ditambahkan ke Total!</p>
            ...
            <div
              class="bg-green-500/20 px-3 py-1 rounded-lg border border-green-500/50"
            >
              <span class="text-white font-bold text-lg"
                >x{{ produk.scanQty }}</span
              >
            </div>
          </div>

          <div
            class="mt-4 pt-4 border-t border-green-500/30 flex justify-between text-sm"
          >
            <span class="text-white font-mono">ID: {{ produk.id }}</span>
            <span class="text-white font-medium">
              + Profit Rp
              {{ (produk.harga - (produk.hpp || 0)).toLocaleString("id-ID") }}
            </span>
          </div>
        </div>
      </div>
      ...
      <button
        v-if="totalPenjualan > 0"
        @click="resetTotal"
        class="w-full mt-6 py-3 text-sm font-semibold text-white transition-colors bg-transparent border border-green-500/50 rounded-xl hover:bg-green-900/60 hover:text-white"
      >
...
const resetTotal = async () => {
  if (barangDitemukan.value.length > 0) {
    try {
      ...
      // Arahkan ke ReceiptView dengan ID transaksi
